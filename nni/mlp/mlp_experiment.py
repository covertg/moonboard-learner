...
def run_trial(params):
    ...
    x_train, x_val, x_test = [np.array([(p.array if params['flattened_input'] else p.array_3d) for p in probs]) for probs in splits]
    ...
    callbacks = [
        ...
        moon.ml.util.IntermediateNNI(x_test, y_test),
        tf.keras.callbacks.TensorBoard(log_dir=os.environ['NNI_OUTPUT_DIR'], write_graph=False)
    ]
    batch_size = params['batch_size']
    max_epochs = params['max_epochs']
    
    # Train, test model and send to NNI
