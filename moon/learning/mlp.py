...
def build_keras_mlp(input_shape, hidden_dim, hidden_layers, hidden_activation, dropout_p, adam_lr, output_type, output_len, y_train=None):
    ...
    if output_type == 'ordinal_coral':
        ...
        metrics = {'probits': [util.mae(), util.accuracy_k(1), util.accuracy_k(0)]}
        name = 'MLP_Coral'
    ...
    elif output_type == 'categorical_xentropy':
        # Standard classifier
        logits = layers.Dense(output_len, name='logits')(features)
        probits = layers.Softmax(name='probits')(logits)
        loss = {'probits': 'categorical_crossentropy'}
        metrics = {'probits': [util.mae(ordi=False), util.accuracy_k(1, ordi=False), util.accuracy_k(0, ordi=False)]}
        name = 'MLP_Categorical_Classifier'
    else:
        raise NotImplementedError(f'Outplut type "{output_type}" not recognized')
